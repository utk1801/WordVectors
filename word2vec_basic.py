...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  #
  # buffer = collections.deque(maxlen=window_size)  # pylint: disable=redefined-builtin
  # if data_index + window_size > len(data):
  #   data_index = 0
  # buffer.extend(data[data_index:data_index + window_size])
  # data_index += window_size
  # for i in range(batch_size // num_skips):
  #   context_words = [w for w in range(window_size) if w != skip_window]
  #   words_to_use = random.sample(context_words, num_skips)
  #   for j, context_word in enumerate(words_to_use):
  #     batch[i * num_skips + j] = buffer[skip_window]
  #     labels[i * num_skips + j, 0] = buffer[context_word]
  #   if data_index == len(data):
  #     buffer.extend(data[0:window_size])
  #     data_index = window_size
  #   else:
  ...
  while ptr < batch_size:
      i = pivot - 1
      j= pivot + 1

      while i>=0 and i>=(pivot-skip_window):
        batch[ptr] = data[pivot]
        labels[ptr] = [data[i]]
        i = i-1
        ptr = ptr + 1

      while j<len(data) and j<= (pivot + skip_window):
        batch[ptr] = data[pivot]
        labels[ptr] = [data[j]]
        j = j + 1
        ptr = ptr + 1
...
def train(sess, model, data, dictionary, batch_size, num_skips, skip_window, 
  ...
  # print("Printing neighbours for {first,american,would}: \n")
  # my_list=['first','american','would']
  # for i in xrange(len(my_list)):
  #   new_word = my_list[i]
  #   top_k1 = 20 #number of similar words needed.
  #   nearest_neighbour = (-sim[i,:]).argsort()[1:top_k1 + 1]
  #   str_out = "Nearest to %s:" % new_word
  #   for k in xrange(0,top_k1,1):
  #     close_words = reverse_dictionary[nearest_neighbour[k]]
  #     str_out = "%s %s," % (str_out,close_words)
  #   print(str_out)
...
if __name__ == '__main__':
  ...
  with tf.Session(graph=graph) as sess:
    ...
    pickle.dump([dictionary, trained_steps, embeddings], open(model_filepath, 'wb'), HIGHEST_PROTOCOL)
